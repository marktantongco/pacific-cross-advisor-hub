    ...
    <div className="space-y-6 pb-8">
      <RevealSection>
        <div className="flex items-center gap-3">
          ...
          <span className="arch-badge arch-badge-owl">Training</span>
        </div>
      </RevealSection>
      ...
      {view === 'presentation' && (
        <div className="space-y-4" data-archetype="owl">
          <RevealSection>
            <div>
          ...
          <RevealSection delay={300}>
            <div className="arch-card-beaver-checklist panel" style={{ background: 'var(--bg)', border: '2px solid var(--accent-red)' }}>
      ...
      {view === 'competitors' && (
        <div className="space-y-4" data-archetype="owl">
          ...
          <RevealSection delay={200}>
            <div className="arch-card-owl-stat panel" style={{ border: '2px solid var(--accent-yellow)', background: 'var(--accent-yellow-dim)' }}>
              <div className="flex items-center gap-2 mb-3">
                <p className="font-sub text-sm" style={{ color: 'var(--accent-yellow)' }}>🏆 PACIFIC CROSS ADVANTAGES:</p>
          ...
          <RevealSection delay={300}>
            <div className="grid grid-cols-1 gap-3">
              {[
                { number: '75+', explanation: 'Years of reliable service', source: 'Pacific Cross Philippines founding' },
                { number: '100', explanation: 'Max entry age — no upper limit', source: 'Blue Royale product terms' },
                { number: 'Global', explanation: 'Worldwide coverage with any provider', source: 'Blue Royale plan documentation' },
              ].map((stat, i) => (
                <div key={i} className="arch-card-owl-stat">
                  <p className="arch-card-owl-stat-number" style={{ color: 'var(--accent-yellow)' }}>{stat.number}</p>
                  <p className="arch-card-owl-stat-explanation" style={{ color: 'var(--text)' }}>{stat.explanation}</p>
                  <p className="arch-card-owl-stat-source" style={{ color: 'var(--text-muted)' }}>{stat.source}</p>
                </div>
              ))}
            </div>
          </RevealSection>
